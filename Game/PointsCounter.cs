namespace BlackJack
{
    static class PointsCounter
    {
      ...
      { CardName.Ace, 11 },
        ...
        //підраховка суми без тузів
        private static int _countNoAcesSum(List<Card> cards)
        {


            //передається список карт

            //Aggregate проходить по всім картам, і підраховує якесь сумарне значення
            //перший параметр у метода Aggregate - це початкове значення
            //в данному випадку 0


            //другий метод -це лямбда, яка викликається для кожної карти

            //для кожної карти ми перевіряємо чи це туз

            //якщо туз -то ми беремо 0, якщо не туз, то ми беремо значення зі словника _cardValueMap

            //і це значення ми додаємо до нашого сумарного значення
